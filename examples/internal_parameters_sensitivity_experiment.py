...
def check_different_parameters():
    axcodes="SPL"
    dataset = "3Dircadb1"
    data_id = 2
    ...
    params = [
        ...
        '_bones_threshold_hu',
        "_spine_filter_size_mm",
       '_lungs_max_density',
       '_diaphragm_level_min_dist_to_surface_mm',
       '_diaphragm_level_min_dist_to_sagittal_mm',
        # "_spine_min_bone_voxels_ratio",

    ]
    param_koeffs = [0.25, 0.5, .75, 1., 1.25, 1.5, 1.75]
    default_values = [
        getattr(bn0, param)
        for param in params
    ]
    ...
    for dataset, data_id in itertools.product(["3Dircadb1"],list(range(1,3))):
        ...
        for (i,j), (param, koeff) in enumerated_product(params, param_koeffs):
            ...
            value = default_values[i]*koeff
            # plt.subplot(121)
            slice_i = 50
            setattr(bn, param, value)
