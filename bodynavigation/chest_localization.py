...
def strict_intensity_filter(data3dr_tmp, strict_floor=300, min_floor=220):
...
def deep_struct_filter(data3dr_tmp, body):
    ...
    def weak_dist_bone(bone_hull, body):
        ...
        return misc.resize_to_shape(ld, ss.orig_shape)
...
def deep_struct_filter_old(data3dr_tmp):
...
def internal_filter(lungs, coronal, body):
    #lungs = ss.get_lungs()
...
def area_filter(data3dr_tmp, body, lungs, coronal):
    min_bone_thr = 220
    ...
    lungs_internal_filter = internal_filter(lungs, coronal, body)
...
def area_mask(img, final_area_filter, body):
    ...
    print_it_all(ss, area_inv, intensity_filter*2, "001")
...
def dist_hull(hull):
    ...
    result = (data3dr_tmp>180) & (d<=8)
    return result


def process_z_axe(ribs, lungs, pattern):
    pixels = list()
    for layer in ribs:
